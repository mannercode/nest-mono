...
> 마이크로서비스 구조인 [nest-mono](https://github.com/mannercode/nest-mono) 프로젝트에 해당하는 결정입니다.
...
- **테스트 시 토픽 생성 문제**: 테스트를 위해 Kafka 컨테이너를 초기화할 때 토픽을 미리 생성해야 한다. 예를 들어 `getCustomer` 메시지를 정의하면 `getCustomer`와 `getCustomer.reply` 두 개의 토픽이 필요하다. 토픽 하나를 생성하는 데 수 초가 걸리므로, 토픽이 많아질수록 대기 시간이 크게 늘어난다. nest-mono에서는 전체 테스트 실행 시 인프라를 초기화하므로 이 문제가 특히 커진다.
