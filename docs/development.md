...
각 테스트마다 고유한 DB 이름을 생성하기 위해 `process.env.TESTLIB_ID`를 사용한다.
...
`src/apps/gateway/` 디렉터리에는 다음 파일이 존재한다.
...
`src/apps/cores/cores.module.ts`의 `imports` 배열에 `FoosModule`을 추가한다. `FoosModule`은 `FoosService`를 `exports`해야 다른 계층에서 사용할 수 있다.
